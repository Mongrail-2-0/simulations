...
char prog_name[] = "mongrail";
char version[] = "mongrail";
char progheader[100];
...
{
  fprintf(stdout, "\n%s\n", progheader);
  fprintf(stdout, "https://github.com/mongrail/mongrail\n");
  fprintf(stdout,"\n");
}

static void program_help()
{
  printf("Usage: ./mongrail -c FILENAME -A FILENAME -B FILENAME -i FILENAME [-o FILENAME]\n\n");
...
{
    printf("Usage: ./mongrail -c FILENAME -A FILENAME -B FILENAME -i FILENAME [-o FILENAME]\n Try './mongrail -h' for more information.\n\n");
  /* printf("Usage: mongrail [OPTION]... FILE\n Try 'mongrail -h' for more information.\n\n"); */
...
chrom_data* data_linked_list_creation(unsigned int *chromosome_index, unsigned int *chromosome_no_loci, double *chromosome_length, double *chromosome_recom_rate, unsigned int no_chromosomes, double *chromosome_markers, unsigned int verbose)
{
...
void read_pop(char *filename, GIOChannel *infile, chrom_data *head, char pop_name, unsigned int verbose)
{
  chrom_data* current;
  current = head;
  char *complete_file;
...
void update_model_indv_prob(chrom_data *data_node, double **matrix, unsigned int no_individual)
{
  ...
  /* printf("Calculating the ancestry probability vector Q for chromosome %d ...\n\n", data_node->chrom_id); */
...
int main(int argc, char **argv)
{
...
#endif
  chrom_data *head, *current;
  head = data_linked_list_creation(chrom_index, chrom_no_loci, chrom_length, chrom_recom_rate, no_chrom, loci, Vflag);
  current = head;
  ...
  read_pop(filenames[1],infile_A,head,population_name[0], Vflag);
  ...
  read_pop(filenames[2],infile_B,head,population_name[1], Vflag);
